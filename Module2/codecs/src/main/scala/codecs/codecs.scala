...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] = {
    @tailrec
    def inner(list: List[Json], acc: List[A] = List.empty[A]): Option[List[A]] = list match {
      case Nil => Some(acc.reverse)
      case x :: xs => decoder.decode(x) match {
        case Some(a) => inner(xs, a :: acc)
        case _ => None
      }
    }

    Decoder.fromFunction {
      case Json.Arr(arr) => inner(arr)
      case _ => None
    }
  }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =Decoder.fromFunction {
    case Json.Obj(fields: Map[String, Json]) =>
      for {
        Json.Str(name) <- fields.get("name")
        Json.Num(age) <- fields.get("age") if age.isValidInt
...
trait ContactsCodecs {
  ...
  implicit def contactsEncoder(implicit encoder: Encoder[List[Person]]): Encoder[Contacts] =
    Encoder.fromFunction { contacts=> Json.Obj(Map("people" -> encoder.encode(contacts.people))) }

  implicit def contactsDecoder(implicit decoder: Decoder[List[Person]]): Decoder[Contacts] =
    Decoder.fromFunction {
      case Json.Obj(fields) =>
        fields
          .get("people")
          .flatMap(decoder.decode)
          .map(peopleList => Contacts(peopleList))
      case _ => None
    }
